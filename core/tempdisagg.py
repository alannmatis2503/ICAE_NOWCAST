...
def disaggregate_annual(
    ...
    hf_indicator : DataFrame d'indicateurs HF (optionnel, pour Chow-Lin)
    ...
    # Tenter Chow-Lin si des indicateurs HF sont disponibles
    if hf_indicator is not None and hf_indicator.shape[1] > 0:
        ...
        if hf.shape[1] > 0 and hf.dropna().shape[0] >= n_hf:
            ...
            X_hf = np.column_stack([np.ones(n_hf), pc1])

            try:
                y_hf = chow_lin(y_a, X_hf, s=s)
