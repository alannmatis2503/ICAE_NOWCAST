    ...
    if "Resultats_Trim" in wb.sheetnames and "quarterly" in results:
        ...
        if dates is not None and icae_col_letter is not None:
            ...
            for idx, (_, row) in enumerate(q_data.iterrows()):
                ...
                _tc = ws_rt.cell(row=r, column=1, value=str(trim_label))
                _tc.border = THIN_BORDER
                _tc.number_format = "@"  # force texte pour éviter interprétation date
                ...
                if rows_in_q and icae_col_letter:
                    ...
                    _c2 = ws_rt.cell(row=r, column=2, value=formula_avg)
                    _c2.border = THIN_BORDER
                    ...
                    if idx >= 4:
                        ...
                        _c3 = ws_rt.cell(row=r, column=3, value=formula_ga)
                        _c3.border = THIN_BORDER
                    ...
                    if idx >= 1:
                        ...
                        _c4 = ws_rt.cell(row=r, column=4, value=formula_gt)
                        _c4.border = THIN_BORDER
...
def _write_icae_from_template(source_path,
    ...
    if "Resultats_Trim" in wb.sheetnames and quarterly is not None:
        ...
        for idx in range(len(q)):
            ...
            # Force le label trimestre en texte pour éviter qu'Excel l'interprète comme date
            cell_t.number_format = "@"

            if rows_in_q and icae_col_letter:
                # ICAE trimestriel = AVERAGE des 3 mois (col B)
                refs = ",".join(f"CALCUL_ICAE!{icae_col_letter}{rr}" for rr in rows_in_q)
                ws_rt.cell(r, 2).value = f"=AVERAGE({refs})"
                ws_rt.cell(r, 2).number_format = "0.00"

                # GA Trim (T/T-4) → col C
                if idx >= 4:
                    r_prev_y = idx - 4 + 2
                    ws_rt.cell(r, 3).value = f"=(B{r}/B{r_prev_y}-1)*100"
                    ws_rt.cell(r, 3).number_format = "0.00"
                else:
                    ws_rt.cell(r, 3).value = '=""'

                # GT Trim (T/T-1) → col D
                if idx >= 1:
                    r_prev = idx - 1 + 2
                    ws_rt.cell(r, 4).value = f"=(B{r}/B{r_prev}-1)*100"
                    ws_rt.cell(r, 4).number_format = "0.00"
                else:
                    ws_rt.cell(r, 4).value = '=""'
            else:
                # Fallback : écrire les valeurs directement (cols B, C, D)
                for j, col_name in enumerate(["icae_trim", "GA_Trim", "GT_Trim"]):
                    val = row.get(col_name)
                    if val is not None and not (isinstance(val, float) and np.isnan(val)):
                        ws_rt.cell(r, j + 2).value = round(float(val), 4) if isinstance(val, (int, float)) else val
                    ws_rt.cell(r, j + 2).number_format = "0.00"
